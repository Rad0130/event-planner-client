...
async function getEvent(id: string): Promise<Event | null> {
  if (!id) {
    console.error("CRITICAL: Attempted to fetch event with undefined ID.");
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`Failed to fetch event ${id}. Status: ${res.status}`);
      return null;
    }

    return res.json();
  } catch (error) {
    console.error(`Error fetching event ${id}. (Is the backend running?):`, error);
    return null;
  }
}
...
// If you want event details to be protected, add this function:
...
export default async function EventDetails({ params }: EventDetailsProps) {
  // Uncomment the line below if you want event details to require login
  // await checkAuth();
  ...
  if (!event) {
    return (
      <div className="max-w-4xl mx-auto py-16 text-center">
        ...
        <p className="text-lg text-gray-600 mt-4">The event you are looking for does not exist or the server is unavailable.</p>
      ...
      <div className="bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8 grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 space-y-6 border-l md:border-t-0 border-gray-200 pl-8 pt-8 md:pt-0">
            ...
            <div className="pt-4">
              <button className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 transition duration-300 shadow-md">
                Request Booking
              </button>
            </div>
          </div>
